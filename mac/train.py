# train.py — Optimized for M4 MacBook (16GB, MPS)
import torch
...
# ─── Hyperparameters tuned for M4 MacBook (16GB) ────────────────────────────────
...
WEIGHT_DECAY = 1e-4
NUM_ITERATIONS = 100
MAX_MOVES = 300  # Cap game length to prevent endless random play
...
def train_step(model, optimizer, replay_buffer, batch_size=TRAIN_BATCH_SIZE):
    ...
    max_per_graph = scatter(policy_logits, batch_idx, reduce='max')
...
def main():
    ...
    for iteration in range(NUM_ITERATIONS):
        ...
        print(f"  ↳ loss={avg_loss:.3f}  win={win_rate:.0%}  "
              f"buf={len(replay_buffer)}  best@iter={best_iteration}  "
